...
public class HistoryCommand {
    ...
    @Command(name = "history", permission = "punishments.commands.history", usage = "punishments history", target = CommandTarget.PLAYER)
        ...
        return ClickableItem.builder()
            .itemStack(
                new ItemStackBuilder(Material.PAPER)
                    .name("§aPunição §f#" + punishment.getId() + "§a.")
                    .lore(
                        "",
                        "§7  Status: §f" + (punishment.isActive() ? "ativa" : "finalizada"),
                        "§7  Duração: §f" + formatDifference(punishment.getPunishmentDuration(), false),
                        "§7  Duração restante: §f" + formatDifference(punishment.getTimeLeft(), true),
                        "§7  Horário da punição: §f" + simpleDateFormat.format(new Date(punishment.getTime())),
                        "§7  Autor: §f" + punishment.getAuthor(),
                        "§7  Tipo da punição: §f" + punishment.getType().getName(),
                        "",
